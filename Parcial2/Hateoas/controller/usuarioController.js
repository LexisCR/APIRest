...
// Create the connection to database
    ...
    connection.query(
        ...
        function(err, results, fields){
            if(err){
                //res.json({Error: "Error en el servidorr"})
                next(err);
            }
            else if(results.length>0){
                var resource = halson({resultado:results})

                .addLink("self", `http://localhost:3000/usuarios`)
                .addLink("eliminar o editar usuario", `http://localhost:3000/usuarios?Id=?`)
                .addLink("departamentos", `http://localhost:3000/departamentos`)
